...
namespace ControleFrotasDLL.DAL.Repositories
{
    public class AluguelRepository : IAluguelRepository
    {
        ...
        public IPagedList<Aluguel> ObterTodosAlugueis(int? pagina)
        {
            ...
            return (_login.Tipo() == null) ? _banco.Alugueis.Include(a => a.ClienteJuridico.Cliente).Include(a => a.ClienteJuridico).Include(a => a.Seguro).Include(a => a.VeiculoEmpresa).Include(a => a.VeiculoEmpresa.Veiculo).Where(a => a.Status == 0).ToPagedList<Aluguel>(numeroPagina, RegistroPorPagina) :
                _banco.Alugueis.Include(a => a.ClienteJuridico.Cliente).Include(a => a.ClienteJuridico).Include(a => a.Seguro).Include(a => a.VeiculoEmpresa).Include(a => a.VeiculoEmpresa.Veiculo).Where(a => _login.Tipo() == a.AluguelClienteId && a.Status == 0).ToPagedList<Aluguel>(numeroPagina, RegistroPorPagina);
        }
